...
public class DataBaseHelper extends SQLiteOpenHelper {
     ...
     * @return
     */
    public String getMaxValue(String device) {
        String  maxvalue = "";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT VALUE  FROM (SELECT * FROM " + TABLE_NAME + " where DEVICE  =  '"
                + device + "'  ORDER BY ID DESC LIMIT 50 )  ORDER BY VALUE DESC LIMIT 1", null);
        while (cursor.moveToNext()) {
            maxvalue =cursor.getString(0);
        }
        return maxvalue;
    }
     ...
     * @return
     */
    public String getMinValue(String device) {
        String  maxvalue = "";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery("SELECT VALUE  FROM (SELECT * FROM " + TABLE_NAME + " where DEVICE  =  '"
                + device + "'  ORDER BY ID DESC LIMIT 50 )  ORDER BY VALUE ASC LIMIT 1", null);
        while (cursor.moveToNext()) {
            maxvalue =cursor.getString(0);
        }
        return maxvalue;
     ...
     * @return
     */
    public float getAverValue(String device) {
        float aver;
        float sum = 0;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("SELECT VALUE   FROM ( SELECT * FROM " + TABLE_NAME + " where DEVICE = '" + device + "' ) ORDER BY ID DESC LIMIT 50 ", null);
        while (res.moveToNext())
            sum += Float.valueOf(res.getString(0));
        aver = sum / res.getCount();
        return aver;
     ...
     * @return
     */
    public String getNewestValue(String device) {
        String newestValue ="";
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor res = db.rawQuery("SELECT VALUE   FROM ( SELECT * FROM " + TABLE_NAME + " where DEVICE = '" + device + "' ) ORDER BY TIME DESC LIMIT 1 ", null);
        while (res.moveToNext())
            newestValue= res.getString(0);
        return newestValue;
    }
